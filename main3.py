...
def url_download(url):
    ...
    try:
        ...
        if r.status_code == 200:
            previous_state = image_atual.copy() if image_atual else None
...
def show_histogram(image):
    ...
    window_histogram = sg.Window('Histograma', layout, finalize=True)
    
    fig, ax = plt.subplots()
    ax.hist(range(256), bins=256, weights=histogram)
    
    canvas_elem = window_histogram['-CANVAS-']
    canvas = FigureCanvasTkAgg(fig, canvas_elem.Widget)
    canvas.get_tk_widget().pack(side='top', fill='both', expand=1)
    
    window_histogram.read(close=True)

def crop_selected_area():
    global image_atual, start_x, start_y, end_x, end_y
    if image_atual and start_x is not None and start_y is not None and end_x is not None and end_y is not None:
        left = min(start_x, end_x)
        right = max(start_x, end_x)
        top = min(start_y, end_y)
        bottom = max(start_y, end_y)
        image_atual = image_atual.crop((left, top, right, bottom))
        show_image()
...
layout = [
    ...
    [sg.Graph(key='-IMAGE-', canvas_size=(500, 500), change_submits=True, drag_submits=True,
              graph_bottom_left=(0, 0), graph_top_right=(400, 400))],
]
